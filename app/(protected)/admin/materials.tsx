...
export default function Materials() { const query = useApi<Record<string, unknown>[]>("/api/materials"); return <Screen><Stack.Screen options={{ title: "Quản lý vật tư" }} /><StateView loading={query.loading} error={query.error} retry={query.refresh}>{query.data && <AdminResource title="Vật tư" endpoint="/api/materials" rows={query.data} refresh={query.refresh} initial={{ code: "", name: "", unit: "m³", group: "Cát" }} fields={[{ key: "code", label: "Mã" }, { key: "name", label: "Tên vật tư" }, { key: "unit", label: "Đơn vị", options: units }, { key: "group", label: "Nhóm", options: groups }]} />}</StateView></Screen>; }
